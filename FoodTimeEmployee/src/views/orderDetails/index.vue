        ...
        <div class="order-middle">
          ...
          <div class="dish-info">
            ...
            <div class="dish-all-amount">
              <label>菜品小计</label>
              <span>￥{{
                (diaForm.amount - diaForm.deliverFee - diaForm.packAmount).toFixed(2)
                }}</span>
            </div>
          </div>
        </div>
          ...
          <div class="amount-info">
            ...
            <div class="amount-list">
              ...
              <div class="send-amount">
                ...
                <span class="amount-price">￥{{ diaForm.deliverFee }}</span>
              </div>
...
export default defineComponent({
  ...
  created() {
    this.init(Number((this.$route.query as any).status) || 0)
  },

  mounted() {
    if (
      (this.$route.query as any).orderId &&
      (this.$route.query as any).orderId !== 'undefined'
    ) {
      this.goDetail((this.$route.query as any).orderId, 2)
    }
    if ((this.$route.query as any).status) {
      this.defaultActivity = (this.$route.query as any).status
    }
  },

  methods: {
    initFun(orderStatus: any) {
      this.page = 1
      this.init(orderStatus)
    },

    change(activeIndex: number) {
    ...
    init(activeIndex: number = 0, isSearch?: any) {
    ...
    getOrderType(row: any) {
      ...
      else if (row.status === 2) return '待接单'
      else if (row.status === 3) return '待派送'
      else if (row.status === 4) return '派送中'
      else if (row.status === 5) return '已完成'
      else if (row.status === 6) return '已取消'
      else return '退款'
    },
    ...
    async goDetail(id: any, status: number, row?: any) {
      ...
      this.row = row || { id: (this.$route.query as any).orderId, status: status }
      if ((this.$route.query as any).orderId) {
        this.$router.push('/order')
      }
    },
...
<style lang="scss">
.dashboard-container {
  .cancelReason { padding-left: 40px; }
  .cancelTime { padding-left: 50px; }
  .orderTime { padding-left: 50px; }
  ...
  .order-dialog {
    ...
    .el-dialog__body { height: 520px !important; }
  }
}

.el-dialog__body { padding-top: 34px; padding-left: 30px; padding-right: 30px; }

.cancelDialog {
  .el-dialog__body { padding-left: 64px; }
  .el-select, .el-textarea { width: 293px; }
  .el-textarea textarea { height: 114px; }
}

.el-dialog__footer {
  .el-checkbox { float: left; margin-left: 40px; }
  .el-checkbox__label { color: #333333 !important; }
}
