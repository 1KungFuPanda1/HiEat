    ...
    <div class="container homecon">
      ...
      <div class="">
        <div v-if="orderData.length > 0">
          <el-table :data="orderData" stripe class="tableBox" style="width: 100%" @row-click="handleTable">
            ...
            <el-table-column label="地址" :class-name="dialogOrderStatus === 2 ? 'address' : ''">
              <template v-slot="{ row }">
                <div class="ellipsisHidden">
                  <el-popover placement="top-start" title="" width="200" trigger="hover" :content="row.address">
                    <template #reference>
            ...
            <el-table-column label="备注">
              <template v-slot="{ row }">
                <div class="ellipsisHidden">
                  <el-popover placement="top-start" title="" width="200" trigger="hover" :content="row.remark">
                    <template #reference>
            ...
            <el-table-column label="操作" align="center"
              :class-name="dialogOrderStatus === 0 ? 'operate' : 'otherOperate'" :min-width="[2, 3].includes(dialogOrderStatus)
                ? 130
              ...
              <template v-slot="{ row }">
                <div class="before">
                  <el-button v-if="row.status === 2" type="text" class="blueBug" @click="
                ...
                <div class="middle">
                  <el-button v-if="row.status === 2" type="text" class="delBut" @click="
                    orderReject(row, $event), (isTableOperateBtn = true)
                    ">
                    拒单
                  </el-button>
                  <el-button v-if="[1, 3, 4, 5].includes(row.status)" type="text" class="delBut"
                    @click="cancelOrder(row, $event)">
                    取消
                  </el-button>
                </div>
                <div class="after">
                  <el-button type="text" class="blueBug non" @click="goDetail(row.id, row.status, row, $event)">
                    查看
                  </el-button>
                </div>
              </template>
    ...
    <el-dialog title="订单信息" v-model="dialogVisible" width="53%" :before-close="handleClose" class="order-dialog">
      ...
      <template v-if="dialogOrderStatus !== 6" #footer>
        <span class="dialog-footer">
          ...
          <el-button v-if="dialogOrderStatus === 2" @click="orderReject(row, $event), (isTableOperateBtn = false)">拒
            单</el-button>
          <el-button v-if="dialogOrderStatus === 2" type="primary"
            @click="orderAccept(row, $event), (isTableOperateBtn = false)">接 单</el-button>
          ...
          <el-button v-if="dialogOrderStatus === 3" type="primary"
            @click="cancelOrDeliveryOrComplete(3, row.id, $event)">派
            送</el-button>
          <el-button v-if="dialogOrderStatus === 4" type="primary"
            @click="cancelOrDeliveryOrComplete(4, row.id, $event)">完
            成</el-button>
          <el-button v-if="[1].includes(dialogOrderStatus)" type="primary"
            @click="cancelOrder(row, $event)">取消订单</el-button>
        </span>
      </template>
    </el-dialog>
      ...
      return [
        {
          label: '待接单',
          value: 2,
          num: (this.orderStatics as any).toBeConfirmed,
        },
        {
          label: '待派送',
          value: 3,
          num: (this.orderStatics as any).confirmed,
        },
      ]
    }
  },
  ...
  methods: {
    async getOrderListData(status: number) {
      ...
      if (
        ...
        data.data.data.records.length > 1
      ) {
        const row = data.data.data.records[0]
...
.order-middle {
  .user-info {
    ...
    .user-info-box {
      ...
      label { margin-right: 17px; color: #666; }
    ...
    .orderCancel {
      ...
      div { padding: 0 10px; background-color: #e5e4e4; }
      span { color: #f56c6c; }
    }
  }
  ...
  .dish-info {
    ...
    .dish-list {
      ...
      .dish-item {
        ...
        .dish-num { margin-right: 51px; }
      }
    }

    .dish-label { margin-right: 65px; }
    ...
    .dish-all-amount {
      ...
      label { color: #333333; font-weight: bold; margin-right: 5px; }
      span { color: #f56c6c; }
    }
  }
}
...
.order-bottom {
  .amount-info {
    ...
    .amount-label { color: #666; margin-right: 65px; }
    ...
    .amount-list {
      ...
      .dish-amount, .package-amount, .pay-type {
        display: inline-block;
        width: 300px;
        margin-bottom: 14px;
        flex: 50%;
      ...
      .send-amount, .all-amount, .pay-time {
        display: inline-block;
        flex: 50%;
        padding-left: 10%;
      }

      .package-amount { .amount-name { margin-right: 14px; } }

      .all-amount {
        .amount-name { margin-right: 24px; }
        .amount-price { color: #f56c6c; }
      }

      .send-amount { .amount-name { margin-right: 10px; } }
    }
  }
}
</style>
<style lang="scss">
.dashboard-container {
  .cancelTime { padding-left: 30px; }
  .orderTime { padding-left: 30px; }
