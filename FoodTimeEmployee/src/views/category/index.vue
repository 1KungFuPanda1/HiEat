<template>
  <div class="dashboard-container">
    <div class="container">
      ...
      <el-table v-if="tableData.length" :data="tableData" stripe class="tableBox">
        ...
        <el-table-column label="操作" width="200" align="center">
          <template v-slot="{ row }">
            <el-button type="text" size="small" class="blueBug" @click="editHandle(row)">修改</el-button>
            <el-button type="text" size="small" class="delBut" @click="deleteHandle(row.id)">删除</el-button>
            <el-button type="text" size="small" class="non" :class="{
              blueBug: row.status == '0', delBut: row.status != '0'
    ...
    <el-dialog v-model="classData.dialogVisible" :title="classData.title" width="30%" :before-close="handleClose">
      <el-form ref="classDataRef" :model="classData" class="demo-form-inline" :rules="rules" label-width="100px">
...
export default defineComponent({
  ...
  methods: {
    ...
    addClass(st: any) {
      if (st == 'class') { this.classData.title = '新增菜品分类'; this.type = '1' }
      else { this.classData.title = '新增套餐分类'; this.type = '2' }
      this.action = 'add'
      this.classData.name = ''
      this.classData.sort = ''
    ...
    handleClose() {
      ...
      const ref = this.$refs.classDataRef as any
    ...
    statusHandle(row: any) {
      ...
      this.$confirm('确认调整该分类的状态?', '提示', {
        ...
        enableOrDisableEmployee({ id: this.id, status: !this.status ? 1 : 0 })
          ...
          .catch(err => { this.$message.error('请求出错了：' + err.message) })
      })
    },
    ...
    deleteHandle(id: any) {
      this.$confirm('此操作将永久删除该分类，是否继续？', '确定删除', {
        ...
        deleCategory(id).then(res => {
          if (res.data.code === 1) { this.$message.success('删除成功！'); this.init() }
    ...
    submitForm(st?: any) {
      const ref = this.$refs.classDataRef as any
      if (this.action === 'add') {
        ref.validate((value: boolean) => {
          if (value) {
            addCategory({ shopId: this.shopId, name: this.classData.name, type: this.type, sort: this.classData.sort })
              .then(res => {
                if (res.data.code === 1) { this.$message.success('分类添加成功！'); ref.resetFields(); if (!st) { this.classData.dialogVisible = false }; this.init() }
        ...
        ref.validate((value: boolean) => {
          if (value) {
            editCategory({ id: this.classData.id, shopId: this.shopId, name: this.classData.name, sort: this.classData.sort })
              .then(res => {
                if (res.data.code === 1) { this.$message.success('分类修改成功！'); this.classData.dialogVisible = false; ref.resetFields(); this.init() }
                else { this.$message.error(res.data.desc || res.data.msg) }
              })
              .catch(err => { this.$message.error('请求出错了：' + err.message) })
          }
...
.dashboard-container {
  margin: 30px;
  .container {
    background: #fff; position: relative; z-index: 1; padding: 30px 28px; border-radius: 4px;
    .tableBar { display: flex; margin-bottom: 20px; justify-content: space-between; }
